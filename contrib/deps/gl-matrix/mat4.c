...
mat4_t mat4_frustum(double left, double right, double bottom, double top, double fnear, double ffar, mat4_t dest);
...
mat4_t mat4_frustum(double left, double right, double bottom, double top, double fnear, double ffar, mat4_t dest) {
    ...
    dest[10] = -(ffar + fnear) / fn;
    dest[11] = -1;
    dest[12] = 0;
    dest[13] = 0;
    dest[14] = -(ffar * fnear * 2) / fn;
    dest[15] = 0;
    return dest;
}

mat4_t mat4_perspective(double fovy, double aspect, double fnear, double ffar, mat4_t dest) {
    double top = fnear * tan(fovy * M_PI / 360.0),
        right = top * aspect;
    return mat4_frustum(-right, right, -top, top, fnear, ffar, dest);
}

mat4_t mat4_ortho(double left, double right, double bottom, double top, double fnear, double ffar, mat4_t dest) {
    if (!dest) { dest = mat4_create(NULL); }
    double rl = (right - left),
        tb = (top - bottom),
        fn = (ffar - fnear);
